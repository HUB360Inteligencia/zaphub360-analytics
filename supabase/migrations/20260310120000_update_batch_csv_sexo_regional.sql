-- Estende update_new_contact_event_if_empty_batch para sexo e regional (importação CSV)
...
BEGIN
  WITH payload AS (
    SELECT
      ...
      nullif(btrim(x.sexo), '') AS sexo_csv,
      nullif(btrim(x.regional), '') AS regional_csv
    ...
    SET
      ...
      sexo = CASE WHEN coalesce(btrim(nce.sexo), '') = '' AND payload.sexo_csv IS NOT NULL THEN payload.sexo_csv ELSE nce.sexo END,
      regional = CASE WHEN coalesce(btrim(nce.regional), '') = '' AND payload.regional_csv IS NOT NULL THEN payload.regional_csv ELSE nce.regional END
    FROM payload
